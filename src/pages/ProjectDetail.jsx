...
export default function ProjectDetail() {
  ...
  return <PageShell><SEO title={`${project.title}${project.slug === 'thryve' ? ' Fitness Project' : project.slug === 'attheblanc' ? ' Web Design' : project.slug === 'qzone' ? ' Auto Detailing Website' : ''} | Kris Dane Madlambayan`} description={project.seoDescription || `${project.description} Read the ${project.title} project case study.`} image={project.cover} />
    ...
    {(project.dataSource || project.notice) && <section className="section case-live-section"><div className="container case-live-cta"><Section label="Data" title="Current anime information.">{project.dataSource && <p>{project.dataSource}</p>}</Section>{project.notice && <Reveal className="case-live-action"><p className="eyebrow"><span />Important notice</p><p>{project.notice}</p></Reveal>}</div></section>}
    {project.liveUrl && <section className="section case-live-section"><div className="container case-live-cta"><Section label="Interface tour" title="A closer look at the product." /><Reveal className="case-live-action"><p>{project.tourCopy || 'Explore the complete project directly to experience its interface, features, and interactions.'}</p><Button href={project.liveUrl}>View website</Button></Reveal></div></section>}
    <section className="section surface-section"><div className="container detail-columns"><Section label="Technology" title={project.stack.length ? 'Built with a focused stack.' : 'Stack details pending verification.'}>{project.stack.length > 0 && <div className="badge-row large-badges">{project.stack.map((item) => <TechBadge key={item}>{item}</TechBadge>)}</div>}</Section><Section label="Reflection" title="What I learned."><p>{project.learning}</p></Section></div></section>
